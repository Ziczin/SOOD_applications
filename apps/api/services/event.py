...
def check_event(user_id, event, other=None):
    ...
    with transaction.atomic():
        ...
        subscriber = EventSubscriber.objects.select_for_update().filter(
            user_id=user_id,
            event=event,
            other=other
        ).first()
...
def set_event_response(event, other, response):
    response_value = json.dumps(response, ensure_ascii=False, cls=DjangoJSONEncoder) if response is not None else None
    
    return EventSubscriber.objects.filter(
        event=event,
        other=other
    ).update(
