...
def parse(rules):
    ...
    for part in rules.split(';'):
        ...
        for expanded_method in _expand_method_keys(method_key):
            if not result.get(expanded_method): result[expanded_method] = set()
            result[expanded_method].update(roles_part.split(','))
    return result

def user_has_role(user, role_name):
    return getattr(user, 'is_authenticated', False) and role_name in set(get_permissions(user) or [])
    ...
    def decorator(view_class):
        ...
        def get_permissions(self):
            ...
            if original_permission_classes:
                return [permission_class() for permission_class in original_permission_classes]
            return [AllowAny()]
