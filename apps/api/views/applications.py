...
def set_event_response(event, other, response):
    now = timezone.now()
    response_text = None if response is None else json.dumps(response, ensure_ascii=False)
    EventSubscriber.objects.filter(event=event, other=other).update(response=response_text, last_check=now)

def check_event_for_user(user, event, other):
    now = timezone.now()
    with transaction.atomic():
        obj = EventSubscriber.objects.select_for_update().filter(user=user, event=event, other=other).first()
        if not obj:
            EventSubscriber.objects.create(user=user, event=event, response=None, other=other, last_check=now)
            return None
...
def on_application_created(application: Application):
    dept_id = None
    if getattr(application, 'form', None) and getattr(application.form, 'department', None):
        dept_id = application.form.department.id
    set_event_response("application-appear", dept_id, _serialize_full_application(application))
...
class ApplicationAPIView(APIView):
    def get(self, request, *args, **kwargs):
        ...
        cache_parts = ['list', dept, created_after, created_before, user, 'short' if short else 'full']
        ...
        serializer_class = ApplicationNoFieldsSerializer if short else ApplicationSerializer
